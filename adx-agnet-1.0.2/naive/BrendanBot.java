 ...
 * 
 */
public class BrendanBot extends Agent {

	private final Logger log = Logger
			.getLogger(BrendanBot.class.getName());
	...
	/*
	 * current ad space bid
	 */
	double persistantBid = 1.0;
	...
	private void handleICampaignOpportunityMessage(
		...
		//bids 1 for each auction
		long cmpBid = Double.doubleToLongBits(Double.MIN_VALUE);

		double cmpBidUnits = cmpBid / 1000.0;
		log.info("This is the value of cmpBid: " + cmpBid);
	...
	protected void sendBidAndAds() {
		...
		for (CampaignData campaign : myCampaigns.values()) {
			...
			*/
			...
			/*
			if (day == 5) {
				log.info("campaign " + campaign.toString() + " has reachImps " + campaign.reachImps);
				log.info("rbid for campaign " + campaign.toString() + " equals " + rbid);
			} */
			
			double rbid = 1000.0 * persistantBid * campaign.budget / campaign.reachImps;
